...
| T7 | A smuggled strategy/performance field or evaluation artifact rides in the proposal | The 35-check verifier scans every proposal document (keys **and** values) for forbidden tokens and refuses any evaluation-output file in the proposal |
| T8 | The automation auto-merges or applies to the accepted branch | Review policy is draft-only / human-required / no-auto-merge / no-retarget / no-accepted-mutation; the descriptor builder and verifier fail closed otherwise; **no** workflow at HEAD grants `contents: write` |
| T9 | A workflow exfiltrates data or gains write/network power | No workflow at HEAD grants `contents: write`, contacts an exchange host, uploads an artifact, references a secret, force-pushes, uses the PR-target event, or enables auto-merge (`test_workflow_security.py`, `test_m3e_workflow_security.py`, and `_no_unsafe_workflow`) |
