"""Crash-recoverable completion intent for the run-003 publication (N6).

The orchestrator publishes the immutable run archive and its aliases as one
durable transaction, then appends the ``completed`` registry event. A crash in
the narrow window *after* publication but *before* that append would leave the
registry stuck at ``started`` with a fully published, verified archive on disk
and no way to finalize without re-running the whole evaluation.

To make that window recoverable without any recomputation, the orchestrator
writes this completion intent durably *before* it publishes: it records the
exact ``completed`` event bytes it will append and the SHA-256 of every file the
publication writes. The calculation-free finalizer
(:mod:`eth_research.m3a_recovery`) can then, and only then, verify that every
recorded artifact is present on disk with the recorded bytes and that the
canonical registry is still exactly ``registered`` -> ``started`` for this id,
and append the recorded ``completed`` event. On a clean run the orchestrator
clears the intent immediately after the append, so its mere presence signals an
unfinalized run.

The recorded ``completed`` line is validated on both write and read: it must
parse as a v2 ``completed`` event for the recorded experiment id whose
``previous_event_sha256`` equals the recorded chain value, so the intent cannot
name one run and carry another's completion.
"""
...
COMPLETION_INTENT_RELPATH: str = "research/m3a/completion_intent.json"
COMPLETION_INTENT_SCHEMA_VERSION: int = 1
...
class CompletionIntent:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            event = parse_registry_line(self.completed_event_line)
        except ValueError as exc:
            raise CompletionIntentError(
                f"completed_event_line is not a valid registry event: {exc}"
            ) from exc
        if not isinstance(event, ExperimentEventV2) or event.event != EVENT_COMPLETED:
            raise CompletionIntentError("completed_event_line is not a v2 'completed' event")
    ...
    @property
    def completed_event(self) -> ExperimentEventV2:
        """Re-parse the recorded ``completed`` event (validated on construction)."""
        event = parse_registry_line(self.completed_event_line)
        if not isinstance(event, ExperimentEventV2):  # pragma: no cover - validated already
            raise CompletionIntentError("completed_event_line is not a v2 event")
        return event
    ...
    def to_json_bytes(self) -> bytes:
        payload = {
            "completion_intent_schema_version": COMPLETION_INTENT_SCHEMA_VERSION,
    ...
    def from_json_bytes(cls, data: bytes) -> CompletionIntent:
        ...
        if version != COMPLETION_INTENT_SCHEMA_VERSION:
...
def write_completion_intent(repo_root: str | Path, intent: CompletionIntent) -> None:
    ...
    durable_write_bytes(Path(repo_root) / COMPLETION_INTENT_RELPATH, intent.to_json_bytes())
...
def read_completion_intent(repo_root: str | Path) -> CompletionIntent | None:
    ...
    path = Path(repo_root) / COMPLETION_INTENT_RELPATH
    if not path.exists():
        return None
...
def clear_completion_intent(repo_root: str | Path) -> None:
    ...
    durable_remove(Path(repo_root) / COMPLETION_INTENT_RELPATH)
