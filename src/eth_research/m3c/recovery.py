"""Calculation-free finalizer for a crashed M3C run publication.

If the orchestrator crashed after publishing the four immutable artifacts but
before it appended the ``completed`` registry event, this tool finalizes the run
without re-running any strategy, backtest, metric, solver, cost model, bootstrap,
or decision rule. It reads the durable completion intent, verifies that every
recorded artifact is present with the recorded bytes and that the registry is
still exactly ``registered`` → ``started`` for the intent's experiment id (with
the started line the intent chains onto), and appends the exact ``completed``
event the intent recorded. It never reconstructs the dataset or recomputes a
result::

    python -m eth_research.m3c.recovery --repo-root . --status
    python -m eth_research.m3c.recovery --repo-root . --finalize

``--status`` is read-only and always exits 0. ``--finalize`` exits 0 only when the
run is finalized (or already was, or there is no pending intent) and non-zero
otherwise, so it is safe to invoke unconditionally: on a clean tree with no
pending intent it reports ``no-intent`` and does nothing.
"""
...
class RecoveryStatus:
    """The assessed recoverable state of a (possibly crashed) M3C run."""
...
def assess(repo_root: str | Path) -> RecoveryStatus:
    """Report whether a pending M3C run can be finalized calculation-free.

    Read-only: it reads the completion intent and the registry, hashes the
    published artifacts, and classifies the state. It never appends.
    """
    root = Path(repo_root)
...
def finalize(repo_root: str | Path) -> RecoveryStatus:
    ...
    append_registry_event(root / M3C_REGISTRY_RELPATH, intent.completed_event)
    clear_completion_intent(root)
    return RecoveryStatus(
        STATE_FINALIZED,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="eth_research.m3c.recovery",
        description="Finalize a crashed M3C-run publication without recomputation.",
    )
    ...
    except (CompletionIntentError, M3CRegistryError, ValueError) as exc:
        # ValueError covers field-level strict-validation failures on a corrupt
        # intent, so the CLI fails closed with a clean message instead of an
        # uncaught traceback.
    ...
    # (finalized just now, already finalized, or no pending intent) succeed; only a
    # genuinely stuck run (not-finalizable / failed) is an error.
