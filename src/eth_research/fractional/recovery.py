"""Calculation-free finalizer for a crashed fractional-run publication (R1).

If the orchestrator crashed after publishing the three immutable artifacts but
before it appended the ``completed`` registry event, this tool finalizes the run
without re-running any strategy, backtest, metric, solver, or cost model. It reads
the durable completion intent, verifies that every recorded artifact is present
with the recorded bytes and that the registry is still exactly
``registered`` → ``started`` for the intent's experiment id (with the started line
the intent chains onto), and appends the exact ``completed`` event the intent
recorded. It never reconstructs the dataset or recomputes a result::

    python -m eth_research.fractional.recovery --repo-root . --status
    python -m eth_research.fractional.recovery --repo-root . --finalize

``--status`` is read-only and always exits 0. ``--finalize`` exits 0 only when
the run is finalized (or already was, or there is no pending intent) and non-zero
...
class RecoveryStatus:
    """The assessed recoverable state of a (possibly crashed) fractional run."""
...
def assess(repo_root: str | Path) -> RecoveryStatus:
    """Report whether a pending fractional run can be finalized calculation-free.
...
def finalize(repo_root: str | Path) -> RecoveryStatus:
    ...
    append_registry_event(root / M3B_REGISTRY_RELPATH, intent.completed_event)
    clear_completion_intent(root)
    return RecoveryStatus(
        STATE_FINALIZED,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="eth_research.fractional.recovery",
        description="Finalize a crashed fractional-run publication without recomputation.",
    )
