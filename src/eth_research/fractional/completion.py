"""Crash-recoverable completion intent for the fractional run publication (R1).

The orchestrator publishes the three immutable artifacts (results JSON, report
Markdown, manifest) as one durable transaction, then appends the ``completed``
registry event. A crash in the narrow window *after* publication but *before*
that append would leave the registry stuck at ``started`` with a fully published,
verified archive on disk and no way to finalize without re-running the whole
75-cell experiment — and, worse, the orchestrator's blanket ``except`` handler
would record an honest-looking ``failed`` event that *mislabels a published
success* (defect R1).
...
(:mod:`eth_research.fractional.recovery`) can then — and only then — verify that
every recorded artifact is present on disk with the recorded bytes and that the
registry is still exactly ``registered`` → ``started`` for this id (with the
started line the intent chains onto), and append the recorded ``completed``
event. On a clean run the orchestrator clears the intent immediately after the
append, so its mere presence signals an unfinalized run.

The recorded ``completed`` line is validated on both write and read: it must
parse as a ``completed`` :class:`FractionalRegistryEvent` for the recorded
experiment id whose ``previous_event_sha256`` equals the recorded chain value, so
...
FRACTIONAL_COMPLETION_INTENT_RELPATH: str = "research/m3b/completion_intent.json"
FRACTIONAL_COMPLETION_INTENT_SCHEMA_VERSION: int = 1
...
class CompletionIntent:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            event = FractionalRegistryEvent.from_json_line(self.completed_event_line + b"\n")
    ...
    def completed_event(self) -> FractionalRegistryEvent:
        ...
        return FractionalRegistryEvent.from_json_line(self.completed_event_line + b"\n")
    ...
    def to_json_bytes(self) -> bytes:
        payload = {
            "completion_intent_schema_version": FRACTIONAL_COMPLETION_INTENT_SCHEMA_VERSION,
    ...
    def from_json_bytes(cls, data: bytes) -> CompletionIntent:
        ...
        if version != FRACTIONAL_COMPLETION_INTENT_SCHEMA_VERSION:
...
def write_completion_intent(repo_root: str | Path, intent: CompletionIntent) -> None:
    ...
    durable_write_bytes(
        Path(repo_root) / FRACTIONAL_COMPLETION_INTENT_RELPATH, intent.to_json_bytes()
    )
...
def read_completion_intent(repo_root: str | Path) -> CompletionIntent | None:
    ...
    path = Path(repo_root) / FRACTIONAL_COMPLETION_INTENT_RELPATH
    if not path.exists():
        return None
...
def clear_completion_intent(repo_root: str | Path) -> None:
    ...
    durable_remove(Path(repo_root) / FRACTIONAL_COMPLETION_INTENT_RELPATH)
