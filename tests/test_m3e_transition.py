...
def _runner_rows(m3e_write_runner, tmp_path, base) -> tuple[list[list[Any]], str]:
...
def test_a_valid_append_only_extension(m3e_write_runner, tmp_path, base) -> None:
...
def test_a_mismatched_new_window_fingerprint_is_rejected(base) -> None:
...
def test_a_gap_after_the_accepted_cohort_is_rejected(base) -> None:
...
def test_an_overlap_with_the_accepted_cohort_is_rejected(base) -> None:
...
def test_an_internally_discontiguous_new_window_is_rejected(base) -> None:
    rows = _canon_rows("2026-07-15T00:00:00Z", 3)
...
def test_transition_hash_is_deterministic(m3e_write_runner, tmp_path, base) -> None:
    rows, fp = _runner_rows(m3e_write_runner, tmp_path, base)
