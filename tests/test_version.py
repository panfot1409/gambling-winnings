...
def test_active_development_version_is_v2c() -> None:
    """The active development version is the V2C pre-release ``2.0.0.dev2``.

    V2C is a candidate-free, data-only prospective-evidence and operational-
    qualification foundation built on the merged V2A and V2B null results. The
    version is a development pre-release (``.dev2``): the V2 research foundation is
    still under construction and nothing is released or tagged as 2.0.
    ``Private :: Do Not Upload`` is preserved and no license is added. Bumping the
    active version from ``2.0.0.dev1`` to ``2.0.0.dev2`` turns V2B into a completed,
    frozen milestone whose committed governed artifacts pin their own recorded
    version (``2.0.0.dev1``, stamped in ``research/v2b/**`` and the V2AB
    accepted-version constants), exactly as every earlier completed milestone does.
    """
...
def test_milestone_4b_is_frozen_at_1_1_0() -> None:
    ...
    cross-asset research-reset milestone frozen at ``2.0.0.dev1``. V2C is the
    current development milestone at ``2.0.0.dev2``. Every completed milestone's
...
def test_completed_milestones_pin_their_own_frozen_versions() -> None:
    ...
    ``2.0.0.dev1``. V2C is the current development milestone and tracks the live
    running version ``2.0.0.dev2``.
    """
