...
def test_milestone_4b_version() -> None:
    """Milestone 4B code identifies itself as 1.1.0.
    ...
    platform 1.0 release candidate (1.0.0). Milestone 4B is the multi-asset
    portfolio research simulator stacked on 4A: the next — additive — version
    1.1.0, still not tagged. Every prior milestone's frozen artifacts still pin
    their own versions and remain byte-identical under the 1.1.0 running
    package, because a running version that differs from a frozen artifact
    version marks a development ("snapshot") run rather than re-authorizing a
    frozen one-time run.
    """
    assert eth_research.__version__ == "1.1.0"
...
def test_completed_milestones_pin_their_own_frozen_versions() -> None:
    ...
    M3C/M3D/M3E/M3F and now 4A are complete: each constant pins the literal
    version its committed artifacts stamp, independent of the running package,
    so every accepted artifact rebuilds byte-for-byte. Milestone 4B is the
    current milestone and tracks the live running version 1.1.0.
    """
