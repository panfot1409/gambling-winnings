...
def _plan_at(base, as_of: str):
    return build_update_plan(base, plan_update_window(base, as_of))


def test_build_tiles_a_multi_day_due_window(base) -> None:
...
def test_build_is_deterministic(base) -> None:
...
def test_idempotency_key_changes_with_the_due_window(base) -> None:
    a = _plan_at(base, "2026-07-22T02:17:00Z")  # 7 buckets
    b = _plan_at(base, "2026-07-23T02:17:00Z")  # 8 buckets
    assert a.idempotency_key != b.idempotency_key


def test_a_large_window_tiles_into_multiple_windows(base) -> None:
...
def test_a_noop_decision_has_no_plan(base) -> None:
...
def test_round_trips_through_from_mapping(base) -> None:
...
def test_load_from_a_committed_path(base, tmp_path: Path) -> None:
    plan = _plan_at(base, "2026-07-22T02:17:00Z")
...
def test_a_forged_idempotency_key_is_rejected(base) -> None:
...
def test_a_forged_plan_hash_is_rejected(base) -> None:
    doc = json.loads(_plan_at(base, "2026-07-22T02:17:00Z").to_json_bytes())
...
def test_first_missing_must_be_one_day_after_last_open(base) -> None:
    doc = json.loads(_plan_at(base, "2026-07-22T02:17:00Z").to_json_bytes())
...
def test_non_canonical_plan_bytes_are_rejected(base, tmp_path: Path) -> None:
    doc = json.loads(_plan_at(base, "2026-07-22T02:17:00Z").to_json_bytes())
    path = tmp_path / "update_plan.json"
...
def test_the_plan_never_reaches_the_forming_candle(base) -> None:
